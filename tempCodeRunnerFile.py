...
def verify_wedding_pin(data: PinVerification, db: Session = Depends(get_db)):
    ...
    return {"valid": wedding.pin.strip() == data.pin.strip()}
...
def create_wedding(wedding: schemas.WeddingCreate, db: Session = Depends(get_db)):
    db_wedding = models.Wedding(**wedding.model_dump())
    db.add(db_wedding)
    db.commit()
...
def get_weddings(db: Session = Depends(get_db)):
    return db.query(models.Wedding).all()
...
def update_wedding(wedding_id: str, wedding: schemas.WeddingUpdate, db: Session = Depends(get_db)):
    ...
    for field, value in wedding.model_dump(exclude_unset=True).items():
...
async def create_photo(
    ...
    # Verify guest and wedding exist
    ...
    try:
        ...
        # Explicitly load the guest relationship
        db.refresh(db_photo, ['guest'])
        
        return db_photo
    except Exception as e:
        # If anything goes wrong, clean up the file if it was saved
...
def get_wedding_memories(wedding_id: str, db: Session = Depends(get_db)):
    # Use joinedload to eagerly load the guest relationship
    photos = (
